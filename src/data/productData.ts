// src/data/poductData.ts
// هذا الملف يحتوي على البيانات الثابتة ويبدأ عملية جلب المخزون الديناميكي عند استيراده وتحديث نفسه.
...
// تعريف البيانات الثابتة الأولية للمنتج
// يتم تضمين حقول المخزون هنا بقيم أولية (فارغة)
export const PRODUCT_DATA: ProductOut = {
    id: 1,
    name: "كوتشي NIKE SV5",
    nameEn: "NIKE SV5",
    label: "كفاءة وأناقة",
...
// دالة تنفيذ غير متزامنة فورية لجلب بيانات المخزون وتحديث PRODUCT_DATA
(async () => {
    const productId = PRODUCT_DATA.id;

    if (productId === undefined || productId === null || isNaN(productId)) {
         console.error("Product ID is missing or invalid in static data. Cannot fetch inventory.");
         return;
    }

    try {
        const { inventory, inventoryIds } = await api.getInventoryByProductId(productId);

        // تحديث خصائص المخزون والمعرفات في الكائن المصدر PRODUCT_DATA
        // هذا النمط يقوم بتعديل الكائن بعد تصديره الأولي
        PRODUCT_DATA.inventory = inventory;
        PRODUCT_DATA.inventoryIds = inventoryIds;

        console.log(`Dynamic inventory data fetched and assigned to PRODUCT_DATA for product ${productId}.`, PRODUCT_DATA.inventory);
        

    } catch (error) {
        console.error(`Failed to fetch dynamic inventory for product ID ${productId}:`, error);
        // في حالة الفشل، تظل inventory و inventoryIds كائنات فارغة كما تم تهيئتها أولياً
    }
})();