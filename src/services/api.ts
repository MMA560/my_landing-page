...
export const api = {
  // --- دوال إدارة المنتجات (Product) ---
  /**
   * يجلب منتجًا واحدًا بواسطة معرفه الفريد.
   * @param productId معرف المنتج.
   * @returns Promise<ProductOut | null> المنتج أو null إذا لم يتم العثور عليه.
   */
  getProductById: async (productId: number): Promise<ProductOut | null> => {
    try {
      ...
      console.error(`فشل في جلب المنتج رقم ${productId}:`, error);
