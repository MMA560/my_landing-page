...
export const api = {
    ...
    // --- دوال API الجديدة للمراجعات والمنتجات ---
    /**
    ...
    /**
     * يجلب منتجًا واحدًا بواسطة معرفه.
     * @param id معرف المنتج.
     * @returns Promise<ProductOut | null>
     */
    getProductById: async (id: number): Promise<ProductOut | null> => {
        try {
            // ملاحظة: المسار /products/ هو نقطة النهاية التي تم تعريفها في الـ FastAPI backend
            const response = await apiService.get<ProductOut>(`/order-app/api/v1/products/${id}`);
            return response.data;
        } catch (error) {
            console.error(`فشل في جلب المنتج رقم ${id}:`, error);
            if (axios.isAxiosError(error) && error.response?.status === 404) {
                return null;
            }
            throw error;
